...
let _dbConnection: any = undefined;
...
class UnitOfWorkFactory {
	public create(callback: Function) {
		...
		switch (config.DbType) {
			case "mock":
				let MockUnitOfWork = require("./mockUOW");
				uow = new MockUnitOfWork();
				return callback(uow);
				break;
			case "mysql":
				let MySqlUnitOfWork = require("./mySQLUOW");
				_dbConnection.getConnection((err: any, connection: any) => {
					if (err) {
